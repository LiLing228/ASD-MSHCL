...
from unsupervised.convs.GraphSAGE_conv import SAGEConv
...
class TUEncoder(torch.nn.Module):
	...
	def forward(self, batch, x, edge_index, edge_attr=None, edge_weight=None):
		...
		for i in range(self.num_gc_layers):
			...
			xs.append(x)
		x = F.normalize(x, dim=1)#xpool [3712,32]
		# compute graph embedding using pooling
		if self.pooling_type == "standard":
			xpool = global_add_pool(x, batch)#[32,32]
	...
	def get_embeddings(self, loader, device, is_rand_label=False):
		...
		with torch.no_grad():
			for data in loader:
				...
				x, _ = self.forward(batch, x, edge_index, edge_weight) #[32,32]
				# if data.y==0:
				# 	asd_array = x.numpy()
				# 	# 保存 NumPy 数组为 .npy 文件
				# 	np.savetxt('/mnt/BAK/A-GCL-main_ABIDEI_AAL1/data/asd/{}.txt'.format(batch.name),asd_array)
				# if data.y==1:
				# 	hc_array = x.numpy()
				# 	# 保存 NumPy 数组为 .npy 文件
				# 	np.savetxt('/mnt/BAK/A-GCL-main_ABIDEI_AAL1/data/hc/{}.txt'.format(batch.name),hc_array)
